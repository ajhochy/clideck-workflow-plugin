...
` : '';
  return `You are the pipeline orchestrator for CliDeck Workflow ${s.title || s.id}.
...
You will open the Draft PR after the FIRST issue's first commit lands. Do not open it earlier (an empty PR is noise). Title = "${s.title || s.id}". Body = the current contents of .clideck-workflow/summaries/${s.id}-summary.md (will be created in Step 7) — for now, a placeholder "Workflow ${s.title || s.id} — in progress".
